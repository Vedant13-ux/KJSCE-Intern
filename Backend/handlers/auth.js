...
exports.signin = async function (req, res, next) {
  ...
  try {
    ...
    }).populate('certificates').populate('experiences').populate('applications').populate('posts').populate('certificates').populate({ path: 'commented', select: '_id image author', populate: { path: 'author', select: 'email fname lname photo' } }).populate({ path: 'liked', select: '_id image author', populate: { path: 'author', select: 'email fname lname photo' } }).exec();
    console.log(user);
