...
exports.signin = async function (req, res, next) {
  ...
  try {
    ...
    }).populate('certificates').populate('events').populate('experiences').populate('applications').populate('posts').populate('projects').populate('achievements').populate("internshipsOffered").populate({ path: 'commented', select: '_id image author', populate: { path: 'author', select: 'email fname lname photo' } }).populate({ path: 'liked', select: '_id image author', populate: { path: 'author', select: 'email fname lname photo' } }).populate({ path: 'members', populate: { path: 'member', select: 'fname lname _id email photo' } }).exec();
    console.log(user);
